#!/usr/bin/env python3
"""
Fix Newer Blocks IPFS Data - Upload proof data for blocks 144+ that were created by external miners
"""
...
def fix_newer_blocks_ipfs():
    """Upload proof data to IPFS for blocks 144+ that need it."""
    print("🚀 Fixing IPFS Data for Newer Blocks (144+)")
    print("=" * 50)
    ...
    print(f"📊 Processing blocks 144 to {latest_height}...")
    
    # Analyze which blocks need fixing (144+)
    accessible_blocks = []
    missing_blocks = []
    
    for i in range(144, latest_height + 1):
    ...
    print(f"\\n🔄 Uploading proof data for {len(missing_blocks)} newer blocks...")
    ...
    print(f"   Success rate: {accessible_after/(latest_height - 143)*100:.1f}%")
